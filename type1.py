...
def type1(a, phi, psi, mu, f):
    ...
    nu2_I2 = simplify(1/(2*a)*integrate(psi.subs(x, xi), (xi, a*t-x,x+a*t)))
    nu2_I3 = simplify(1/(2*a)*(
        integrate(integrate(f.subs({x: xi, t : tau}), (xi, x-a*(t-tau),x+a*(t-tau))), (tau, t-x/a, t)) +
        integrate(integrate(f.subs({x: xi, t : tau}), (xi, a*(t-tau)-x,x+a*(t-tau))), (tau, 0, t-x/a))
    ))
    nu2 = simplify(nu2_I1 + nu2_I2 + nu2_I3)

    omega = simplify(mu.subs(t, t - x/a))
    equation = fr"""
    \begin{{align}}
    \text{{Умова задачі}}
    \end{{align}}
